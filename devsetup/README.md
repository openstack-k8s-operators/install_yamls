...
* `CRC_URL` and `KUBEADMIN_PWD` can be used to change requirements for CRC install. `KUBEADMIN_PWD` is auto-generated in `../.secrets.env` (run `make secrets` from the repo root, or it is created automatically by `make input`). Override it with an env var if needed.
...
```commandline
curl -u admin:$BMAAS_REDFISH_PASSWORD http://sushy-emulator.apps-crc.testing/redfish/v1/Systems/
```
```json
{
...
- name: crc-bmaas-01
  ...
  driver_info:
    ...
    redfish_password: <auto-generated, see .secrets.env>
  ports:
...
```bash
oc login -u admin -p $OCP_ADMIN_PASSWD https://api.sno.lab.example.com:6443
```
